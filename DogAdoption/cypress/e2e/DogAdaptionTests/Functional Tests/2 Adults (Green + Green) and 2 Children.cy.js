...
describe('2 Adults and 2 Children', ()=>{
    ...
    it('And add parent 1', ()=>{
        cy.fixture('family-pool').then((data)=>{
            cy.request({
                ...
                body: {
                    "familyId": familyId,
                    "firstName": data.adults[1].name,
                    "lastName": data.familyName,
                    "favouriteColour": data.adults[1].favouriteColour
                }
            }).then((res)=>{
                expect(res.status).to.equal(201)
                expect(res.body).has.property('id').is.not.null
                expect(res.body).has.property('familyId',familyId)
                expect(res.body).has.property('firstName', data.adults[1].name)
                expect(res.body).has.property('lastName', data.familyName)
                expect(res.body).has.property('favouriteColour', data.adults[1].favouriteColour)
            })
        })
    })
    ...
    it('Then the family should be eligible to adopt 1 dog', ()=>{
        cy.request({
            method: 'GET',
